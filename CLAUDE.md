...
- `repository/` — `ExpenseRepository` + `ExpenseSpecifications` (JPA Criteria specs for dynamic filtering).
- `service/` — `ExpenseService` business logic; `ExpenseNotFoundException` (thrown on missing id, mapped to 404 by the global handler).
- `web/` — `@RestController`s (`ExpenseController`, `CategoryController`, `HealthController`), thin layers over the service. Subpackages: `web/dto/` (`ExpenseRequest`/`ExpenseResponse`), `web/mapper/` (`ExpenseMapper`, entity↔DTO), `web/error/` (`ApiError` + `GlobalExceptionHandler`).
